...
import { z } from "zod"
import { FormAddProfileProps } from "./FormAddProfiles.types"
...
import { dataProfilesImmages } from "./FormAddProfile.data"
    ...
    const onSubmit=async (values: z.infer<typeof formSchema>) =>{
        try {
           ...
           if(response.status !== 200){
            toast({
                title: "Error al crear el perfil", variant: "destructive"
            });
        }else{
                toast({
                    ...
                    render={({ field }) => (
                        <FormItem className="space-y-3">
                            ...
                            <FormControl>
                                ...
                                >
                                    {dataProfilesImmages.map((data) => (
                                        <FormItem key={data.urlImage}
                                            className="flex felex-col-reverse justify-center items-center space-x-5 space-y-0 cursor-pointer">

...
export function Profiles(props: ProfileProps) {
    ...
    const deleteUser = async (userIdNetflix: string) => {
        try {
            ...
            console.log(error);
            toast({
                title: "Error al eliminar el perfil", variant: "destructive"
            })
        }
                ...
                {users.map((user) => (
                    <div key={user.id} className="text-center relative cursor-pointer" onClick={() => onClickUser(user)}>
                        <Image src={user.avatarUrl || ""}
                            alt={'ProfileImage ${user.profileName} '}
                            width={140}
                            height={140}
                            className={cn(manageProfiles ? "blur-md" : "", " border-transparent hover:border-2 hover:border-white rounded-md")}
                        />
                            ...
                            <AlertDialog>
                                ...
                                <AlertDialogContent className="bg-zinc-900">
                                    ...
                                    <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction className="text-red-500 border-red-500 border"

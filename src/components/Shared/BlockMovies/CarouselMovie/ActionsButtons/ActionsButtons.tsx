  ...
  return (
    <div className="flex justify-between mb-5">
      <div className="flex gap-2">
        ...
        {isMyList ? (
          <Button size="icon" variant={"ghost"} className="bg-zinc-900 border-2 border-gray-500 rounded-full flex items-center justify-center h-7 w-7 "
          onClick={() => onRemoveFromList()}>
            <X width={10} height={10} className="text-slate-50 fill-zinc-900 h-3 w-3 hover:text-zinc-900">

            </X>

          </Button>
        ): (
          <Button size="icon" variant={"ghost"} className="bg-zinc-900 border-2 border-gray-400 rounded-full flex items-center justify-center h-7 w-7 " onClick={() => onAddToList()}>
